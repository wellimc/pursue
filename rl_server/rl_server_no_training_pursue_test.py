...
CLIENT_1_MOS =   {300: 2, 750: 3, 1200: 4 ,1850:5, 2850:6, 2850:6, 4300:6}
CLIENT_2_MOS =   {300: 1, 750: 2, 1200: 3 ,1850:5, 2850:6, 2850:6, 4300:6}
CLIENT_3_MOS =   {300: 1, 750: 1, 1200: 2 ,1850:3, 2850:4, 2850:5, 4300:6}
CLIENT_4_MOS =   {300: 1, 750: 1, 1200: 1 ,1850:2, 2850:3, 2850:4, 4300:5}
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                video_cout = self.input_dict['video_chunk_coount'];
                last_quality = VIDEO_BIT_RATE[post_data['lastquality']]
                ...
                if video_cout > 0: 
                    model = loadmodel(PER_MODEL)
                    bandwidth = post_data['bandwidthEst']
                    #row = bandwith,delay,jitter,content_type,complexity,resolution,bitrate,device,ram    
                    bitrate = VIDEO_BIT_RATE[post_data['lastquality']]
                    memory_consup = MEMORY_DIC[bitrate]  
                    buffer_size = post_data['buffer']
                    memory_load =   memory_consup *  buffer_size
                    percent_memory=  memory_load / CLIENT_MEMORY[CLIENT_ID]
                    rebuffer_time_log = str(rebuffer_time / M_IN_K)
                    vmos = CLIENT_1_MOS[bitrate]
                    if (percent_memory + memory_ini)  > 0.8:
                        rmos = 0
                    row =  [CLIENT_ID,CLIENT_ID,1,1,bandwidth,last_quality,post_data['buffer'],0,6,10,memory_load,percent_memory]
                    print(row)
                    output = predict(row,model)
                    predict_rmos = round(output[0][1])
                    predict_vmos = round(output[0][0])
                    print('predicted rmos :'+str(predict_rmos))
                    print('predicted  vmos :'+str(predict_vmos))  

                    predict_vmos = vmos
                    predict_rmos = rmos
                    print('corrected  rmos :'+str(predict_rmos))
                    print('corrected  vmos :'+str(predict_vmos))  
                ...
                #reward = reward_quality- 0.1 * (post_data['RebufferTime'] - self.input_dict['last_total_rebuf'])
                ...
                if  rmos == 0 or  ( vmos != 3 or vmos != 4):
                        reward_quality =   (reward_quality * -1)

                print('Reeard Quality-->'+str(reward_quality))
                ...
                if video_cout > 0: 
                    with open(LOG_FILE_PER, 'a') as log_per:   
                        line = str(self.input_dict['video_chunk_coount'])+';'+'user1;android;movie;1;'+str(post_data['bandwidthEst'])+';'+str(VIDEO_BIT_RATE[post_data['lastquality']])+';'+str(post_data['buffer'])+';'+str(rebuffer_time)+';'+'6'+';'+'10'+';'+str(memory_load)+';'+str(percent_memory)+';'+str(vmos)+';'+str(rmos)
                        log_per.write(line) 
                        log_per.write('\n') 
                        log_per.flush()
