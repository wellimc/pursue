...
class PerActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        # Create the actor network
        self.inputs, self.out = self.create_actor_personalzied_network()

        # Get all network parameters
        self.network_params = \
            tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='actorP')
    ...
    def create_actor_personalzied_network(self):
        with tf.variable_scope('actorP'):
            inputs = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])

            split_0 = tflearn.fully_connected(inputs[:, 0:1, -1], 128, activation='relu')
            split_1 = tflearn.conv_1d(inputs[:, 2:3, :], 128, 4, activation='relu')
            split_2 = tflearn.conv_1d(inputs[:, 3:4, :], 128, 4, activation='relu')
            split_3 = tflearn.conv_1d(inputs[:, 4:5, :A_DIM], 128, 4, activation='relu')
            split_4 = tflearn.fully_connected(inputs[:, 5:6, -1], 128, activation='relu')

            split_1_flat = tflearn.flatten(split_1)
            split_2_flat = tflearn.flatten(split_2)
            split_3_flat = tflearn.flatten(split_3)

            merge_net = tflearn.merge([split_0, split_1_flat, split_2_flat, split_3_flat, split_4], 'concat')
